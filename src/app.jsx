...
function buildMath(g,count=10){const pool=g===6?[gInteger,gOrder,gFrac,gPercent,gProp,gEq,gGeo,gWord]:g===7?[gInteger,gOrder,gFrac,gPercent,gProp,gEq,gGeo,gWord]:[gInteger,gOrder,gFrac,gPercent,gProp,gEq,gGeo,gExp,gWord];
 const out=[],seen=new Set();let guard=0;
 while(out.length<count&&guard<200){guard++;const p=pick(pool)(g);if(seen.has(p.prompt))continue;seen.add(p.prompt);out.push({id:out.length,...p});}return out;}
...
const MOT={
 ...
 designer:{label:"Designer",emoji:"🎨",blurb:"You love unlocking gear and making things your own.",reward:"Spend coins to unlock avatars, colors, and themes."},
...
const QUIZ=[
 ...
 {q:"Pick your vibe:",a:[["Champion 🏆","champion"],["Collector 🎖️","collector"],["Designer 🎨","designer"],["Treasure hunter 🎁","explorer"]]},
];
const FACES=["🦊","🐉","🐼","🦅","🐺","🦈","🐯","👾","🤖","🦖","⚡","🔥","🦁","🐙","🌟","🚀"];
const FREE_FACES=["🦊","🐼","🐯","👾"];
const COLORS=["#7c5cff","#ff5c8a","#26c6da","#ffb020","#3ddc84","#ff7043","#5c6bc0","#ec407a"];
const FREE_COLORS=["#7c5cff","#26c6da","#ffb020"];
const THEMES=[{id:"violet",label:"Violet",a:"#7c5cff",a2:"#b39dff",cost:0},{id:"sunset",label:"Sunset",a:"#ff5c8a",a2:"#ffb020",cost:40},{id:"mint",label:"Mint",a:"#26c6da",a2:"#3ddc84",cost:40},{id:"ember",label:"Ember",a:"#ff7043",a2:"#ffca28",cost:60},{id:"galaxy",label:"Galaxy",a:"#5c6bc0",a2:"#ec407a",cost:80}];
...
const BADGES=[
 {id:"first",emoji:"🌱",label:"First Day",test:p=>p.totalDays>=1},
 {id:"s3",emoji:"🔥",label:"3-Day Streak",test:p=>p.streak>=3},
 {id:"s7",emoji:"⚔️",label:"7-Day Streak",test:p=>p.streak>=7},
 {id:"s14",emoji:"👑",label:"14-Day Streak",test:p=>p.streak>=14},
 {id:"reader",emoji:"📚",label:"Bookworm (10 readings)",test:p=>p.readCount>=10},
 {id:"perfect",emoji:"🎯",label:"Perfect Math",test:p=>p.perfectCount>=1},
 {id:"clean",emoji:"✅",label:"7 School-work Days",test:p=>p.swStreak>=7||p.swCount>=7},
 {id:"ten",emoji:"🏅",label:"10 Days Logged",test:p=>p.totalDays>=10},
];
...
const DEFAULT_PROGRESS={xp:0,coins:0,streak:0,lastDate:null,totalDays:0,perfectCount:0,bestMath:0,readCount:0,recentReads:[],swLastDate:null,swStreak:0,swCount:0,swLog:[],badges:[],unlockFaces:[...FREE_FACES],unlockColors:[...FREE_COLORS],unlockThemes:["violet"],theme:"violet",redemptions:[],shareCards:[]};
...
export default function Quest(){
 ...
 const regen=useCallback(()=>{
  if(!profile)return;
  setMath(buildMath(profile.mathGrade,10));
 ...
 function finishReading(){if(readDone||!reading)return;setReadDone(true);
  ...
  readStart.current=0;
  setProg(p=>({...p,xp:p.xp+35,coins:p.coins+15,readCount:p.readCount+1}));
 ...
 function finishMath(){if(mathDone)return;
  ...
  mathStart.current=0;
  setProg(p=>({...p,xp:p.xp+xp,coins:p.coins+coins,perfectCount:p.perfectCount+(perfect?1:0),bestMath:Math.max(p.bestMath,score)}));
  setMathDone(true);
 ...
 function finishSchoolwork(){if(swDoneToday)return;if(!sw.portal||!sw.homework){flashToast("Check both boxes first");return;}
  ...
  setProg(p=>({...p,xp:p.xp+20,coins:p.coins+15,swLastDate:today(),swStreak:nextStreak,swCount:p.swCount+1,swLog:[{date:today(),note:sw.note||"All clear",missing:!!sw.note.trim()},...p.swLog].slice(0,40)}));
 ...
 useEffect(()=>{if(readDone&&mathDone&&!completedToday){setCompletedToday(true);
  ...
  setProg(p=>{const np={...p,xp:p.xp+dayXp,coins:p.coins+dayCoins,streak:nextStreak,lastDate:today(),totalDays:p.totalDays+1};return applyBox(np,box);});
  logEvent({type:"day_complete",streak:nextStreak,allThree:swDoneToday});
  ...
  if(profile.motivator==="star")setProg(p=>({...p,shareCards:[...p.shareCards,{date:today(),streak:nextStreak,level:Math.floor(p.xp/100)+1}]}));
  setCelebrate({xp:dayXp,coins:dayCoins,newBadges,box,streak:nextStreak,allThree:swDoneToday});
 ...
 function buyFace(f){if(prog.unlockFaces.includes(f)){setProfile({...profile,avatar:{...profile.avatar,face:f}});return;}if(prog.coins<20){flashToast("Need 20 🪙");return;}setProg(p=>({...p,coins:p.coins-20,unlockFaces:[...p.unlockFaces,f]}));setProfile({...profile,avatar:{...profile.avatar,face:f}});flashToast("Unlocked! −20 🪙");}
 function buyColor(c){if(prog.unlockColors.includes(c)){setProfile({...profile,avatar:{...profile.avatar,color:c}});return;}if(prog.coins<15){flashToast("Need 15 🪙");return;}setProg(p=>({...p,coins:p.coins-15,unlockColors:[...p.unlockColors,c]}));setProfile({...profile,avatar:{...profile.avatar,color:c}});flashToast("Unlocked! −15 🪙");}
 function buyTheme(t){if(prog.unlockThemes.includes(t.id)){setProg(p=>({...p,theme:t.id}));return;}if(prog.coins<t.cost){flashToast(`Need ${t.cost} 🪙`);return;}setProg(p=>({...p,coins:p.coins-t.cost,unlockThemes:[...p.unlockThemes,t.id],theme:t.id}));flashToast("Theme unlocked!");}
 function redeem(it){if(prog.coins<it.cost){flashToast(`Need ${it.cost} 🪙`);return;}logEvent({type:"redeem",item:it.label,cost:it.cost});setProg(p=>({...p,coins:p.coins-it.cost,redemptions:[{...it,date:today(),claimed:false},...p.redemptions]}));flashToast(`Redeemed: ${it.label}`);}

 if(!ready)return <div className="boot" style={{"--accent":THEMES[0].a}}><div className="boot-in">Loading…</div></div>;
 if(!profile)return <Onboarding onDone={pf=>setProfile(pf)}/>;
 const rootStyle={"--accent":theme.a,"--accent2":theme.a2};

 return(<div className="app" style={rootStyle}>
  ...
  <main className="scroll">
   {screen==="home"&&<Home {...{profile,prog,level,readDone,mathDone,swDoneToday,setScreen,setDayKey}}/>}
   ...
   {screen==="rewards"&&<Rewards {...{profile,prog,store,buyFace,buyColor,buyTheme,redeem,level}}/>}
  ...
  {celebrate&&<Celebrate {...{celebrate,profile,onClose:()=>setCelebrate(null)}}/>}
 </div>);
}
...
function Onboarding({onDone}){
 ...
 const[name,setName]=useState("");const[face,setFace]=useState("🦊");const[color,setColor]=useState("#7c5cff");const total=QUIZ.length;
 const motivator=Object.entries(scores).sort((a,b)=>b[1]-a[1])[0][0];
 return(<div className="app onb" style={{"--accent":"#7c5cff","--accent2":"#b39dff"}}>
  ...
  {step===total+1&&(<div className="onb-card"><h2 className="onb-q center">Make it yours</h2><div className="avatar-preview"><Avatar face={face} color={color} size={88}/></div>
   ...
   <button className="cta" disabled={!name.trim()} onClick={()=>onDone({setup:true,name:name.trim(),avatar:{face,color},motivator,mathGrade:7,lexile:830,portalUrl:""})}>Start →</button>
   <p className="onb-foot">A grown-up sets the exact reading & math level in the Parent tab.</p></div>)}
 </div>);
}

function Home({profile,prog,level,readDone,mathDone,swDoneToday,setScreen,setDayKey}){
 const m=MOT[profile.motivator];
 const spot=profile.motivator==="champion"?`Level ${level} · best math ${prog.bestMath}/10`:profile.motivator==="collector"?`${prog.badges.length}/${BADGES.length} badges earned`:profile.motivator==="designer"?`${prog.coins} 🪙 to spend on gear`:profile.motivator==="earner"?`${prog.coins} 🪙 toward your rewards`:profile.motivator==="explorer"?`Finish today to open a mystery box`:`${prog.streak}-day streak — show it off`;
 return(<div className="page">
  ...
  {readDone&&mathDone&&(<div className="day-done"><div className="dd-emoji">{swDoneToday?"🌟":"✅"}</div>
   ...
   <button className="cta small" onClick={()=>{setDayKey(k=>k+1);setScreen("home");}}>New set (extra practice)</button></div>)}
 </div>);
}
...
function Rewards({profile,prog,store,buyFace,buyColor,buyTheme,redeem,level}){
 const m=profile.motivator;
 ...
 return(<div className="page"><h1 className="rw-title">Rewards</h1>
  <div className="rw-tabs">{tabs.map(t=>(<button key={t} className={`rwtab ${view===t?"on":""}`} onClick={()=>setView(t)}>{labels[t]}</button>))}</div>
  {view==="progress"&&(<div className="rw-body"><div className="bigstat"><span className="bs-num">{level}</span><span className="bs-lbl">Level</span></div>
   ...
   <div className="statline">🎯 Best math score: {prog.bestMath}/10</div><div className="statline">📋 School-work days: {prog.swCount}</div><div className="statline">🪙 {prog.coins} coins</div></div>)}
...
function Parent({profile,setProfile,store,setStore,prog,setProg,events,setEvents,flashToast}){
 ...
 return(<div className="page"><h1 className="rw-title">Parent settings</h1>
  ...
  <div className="psec"><p className="pick-lbl">Motivation profile</p><div className="mot-now">{MOT[profile.motivator].emoji} {MOT[profile.motivator].label} — {MOT[profile.motivator].blurb}</div><button className="ghost-btn" onClick={()=>setProfile(null)}>Retake the quiz</button></div>
...
function Celebrate({celebrate,profile,onClose}){
 const[opened,setOpened]=useState(false);const m=profile.motivator;const box=celebrate.box;
 const boxText=box.t==="coins"?`+${box.v} 🪙`:box.t==="face"?`New avatar: ${box.label}`:box.t==="color"?`New color unlocked`:box.t==="theme"?`New theme: ${box.label}`:`+20 🪙`;
 return(<div className="modal-bg" onClick={onClose}><div className="modal" onClick={e=>e.stopPropagation()}>
  <div className="cel-emoji">{celebrate.allThree?"🌟":MOT[m].emoji}</div><h2 className="cel-title">{celebrate.allThree?"Perfect day!":"Day complete!"}</h2>
  ...
  <div className="mystery">{!opened?<button className="box-btn" onClick={()=>setOpened(true)}>🎁 Tap to open your mystery box</button>:<div className="box-open">🎉 {boxText}</div>}</div>
  <button className="cta" onClick={onClose}>Awesome →</button></div></div>);
}
